...
//               <button className="bg-green-700 hover:bg-purple-50 rounded-lg text-white transition duration-300 ease-linear mt-5 border-2 border-green-600 font-bold hover:text-green-700 p-3 text-xl" onClick={handleCheckout}>CheckOut Now</button>
//             </div>
//           </div>
//         </div>
//       ) : (
...
import CartItem from "../components/CartItem";
import API from "../api";
import { clear } from "../redux/Slices/CartSlice"; // 👈 ADD THIS
...
const navigate = useNavigate();
  ...
  const handleCheckout = async () => {
    try {
      ...
      dispatch(clear()); // 👈 cart empty after order
      navigate("/success");
    } catch (error) {
      console.log(error);
